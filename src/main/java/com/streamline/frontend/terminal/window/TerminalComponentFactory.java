...
public class TerminalComponentFactory {
    ...
    /**
     * Formats text to fit within a fixed length, ensuring it matches the panel width for results.
     * Uses the terminal size and button width to calculate appropriate text length.
     */
    private String formatTextToFixedLength(String text, int maxLength) {
        ...
        // Ensure maxLength doesn't exceed the available terminal width
        int actualMaxLength = Math.min(maxLength, terminalSize.getColumns() - 4); // Account for margins
        
        if (text.length() > actualMaxLength) {
            // Truncate and add ellipsis, ensuring we don't exceed bounds
            int truncateLength = Math.max(0, actualMaxLength - 3);
            return text.substring(0, truncateLength) + "...";
        }
        
        if (text.length() < actualMaxLength) {
            // Pad with spaces to maintain consistent formatting
            return text + " ".repeat(actualMaxLength - text.length());
        }
